---
title: "Central Park Squirrel Census"
description: "**An Example of Data Wrangling Using ggplot**"
format: html
title-block-banner: "media/squirrel.png"
title-block-banner-color: white


#Remove messages/warnings
code-fold: true
execute: 
  warning: false
  message: false
  
#citation
  links:
    - text: NYC OpenData
      href: https://data.cityofnewyork.us/Environment/2018-Central-Park-Squirrel-Census      -Squirrel-Data/vfnx-vebw/about_data
    - text: The Squirrel Census
      href: https://www.thesquirrelcensus.com/
---

The Central Park Squirrel Census offers a uniqely fun dataset to practice with regular expressions.[^1] To find out more for yourself about these squirrels, visit NYC OpenData.[^2] To start, the census includes data on location when sighted—let's find out where these squirrels hang out:

[^1]: The Squirrel Census. (n.d.). *The Squirrel Census*. <https://www.thesquirrelcensus.com/>

[^2]: New York City Department of Parks and Recreation. (2018). *2018 Central Park Squirrel Census — Squirrel Data*. NYC Open Data. <https://data.cityofnewyork.us/Environment/2018-Central-Park-Squirrel-Census-Squirrel-Data/vfnx-vebw/about_data>

```{r}
#| out-width: 70%
#| fig-align: center
#| fig-alt: "A bar chart showing whether squirrels are cited on the ground or above."

library(tidyverse)

#fun dataset from NYC OpenData living in my github repository
SquirrelCensus <- read_csv("https://raw.githubusercontent.com/foxkloske/foxkloske.github.io/main/2018_Central_Park_Squirrel_Census_-_Squirrel_Data.csv")

Squirrel1 <- SquirrelCensus |>
  group_by(Location) |>
  #na.rm not recognizing empty values, so instead tested if any characters are in the value.
  filter(str_detect(Location, "."))

ggplot(Squirrel1, aes(x = Location, fill = Location)) +
  geom_bar(na.rm = TRUE) +
    labs(
      title = "Where Do Squirrels Spend Their Time?",
      x = "",
      y = "Number of Observations"
  ) +
  theme_classic() +

#Centered format
theme(legend.position = "none")
```

Most squirrels are spotted on the ground. These built-in columns gives some detail about where these acorn-lovers spend their days, but we can do better! Let's combine this location data by activity. Here I reshape the activity columns into a longer format so I can compare how often different activities (running, foraging, etc.) appear, and whether they happen on trees or on the ground.

```{r}
#| out-width: 70%
#| fig-align: center
#| fig-alt: "A bar chart showing the distribution of activities for squirrels and whether they are on or near a tree."

Squirrel2 <- SquirrelCensus |>
  #A differnt str command to get rid of empty values!
  filter(str_length(`Specific Location`) > 0) |>
  mutate(tree_status = str_detect(tolower(`Specific Location`),"tree")) |>
  #Combines the five columns into one (x-axis)
  pivot_longer(cols = Running:Foraging,
               names_to = "Activity",
               values_to = "Status") |>
  filter(Status)

ggplot(Squirrel2, aes(x = Activity, fill = tree_status)) +
  geom_bar() +
  labs(
    title = "What Are Squirrels Doing All Day?",
    x = "",
    y = "Number of Observations",
    fill = "On a tree?"
  ) +
  theme_classic()

#Complicated way to sneak in more str() & regular expressions
#str_count() returns line by line, sum returns single value
#The word street also appears in the dataset, but want to exclude from count of tree
tree_count <- sum(str_count(tolower(SquirrelCensus), "(?<=\\s|^)tree"), na.rm = TRUE)
#Number is writen inline with `r tree_count`
```

Much better! The data shows a mix of squirrel-tivities. But most of all, squirrels spend a *tree*-mendous amount of time on and around trees. For this graph, only observations with specific location details are included. However, there are `r tree_count` mentions of tree-related shenanagins in the data set! Let's now get to the *nuts* and bolts of who they spend their time with.

```{r}
#| out-width: 70%
#| fig-align: center
#| fig-alt: "A bar chart of days during the survey when squirrels were most mentioned with dogs."

Squirrel3 <- SquirrelCensus |>
  #Sometimes the same dog in mentioned twice in interaction. Ex: dog was chasing but got dog got lost.
  mutate(num_dogs = str_count(tolower(`Other Interactions`), "(?<!dog)dog"),
         Date = as.character(Date),
         Day = str_sub(Date, 3, 4)) |>
  group_by(Day)

ggplot(Squirrel3, aes(x = Day, y = num_dogs)) +
  geom_col(fill = "springgreen4") +
  labs(
    title = "Daily Encounters of Squirrels and Dogs",
    x = "Day in January",
    y = "Number of Observations",
  ) +
  theme_classic()

#display number in text below
chase_count <- sum(str_count(tolower(SquirrelCensus), "nut|acorn"), na.rm = TRUE)
```

Dogs! These poor squirrels spend their time in Central Park running from dogs! Data is taken over the month of January 2018, but not every day is captured, creating this funky x-axis. January 13 and 14th are the Saturday and Sunday, respectively—these poor squirrels don't get the weekends off. And, to make matters worse, of the 3,022 observations, only `r chase_count` mention eating yummy food!

Thanks for tagging along on this journey to understand the busy social lives of our furry neighbors!

***
*Header Image*: Town and Country: <https://town-country-pest-control.co.uk/common-pests/squirrel-removal/>
